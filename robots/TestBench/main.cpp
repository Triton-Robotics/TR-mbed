...
//    pitch.setPositionOutputCap(100000);
    ...
    */
    ...
    unsigned long timeBefore = us_ticker_read();
    // printff("p_theory,current,chassis_power\n");
    // printfESP("p_theory,current,chassis_power\n"); //,LFpower,LBpower,RFpower,RBpower,LFrpm,LBrpm,RFrpm,RBrpm
    ...
    while (true)
    {
        ...
        float read = curr.read();
        samples[samplePointer++] = read;

        if(samplePointer == SAMPLE_NUM){
            samplePointer = 0;
        }
        ...
        if ((timeStart - loopTimer) / 1000 > 25){
            ...
            if (refLoop >= 5){
                ...
                float chassis_power = power_heat_data.chassis_power; 
                // if (x) {
                    //printff("%.3f,%.3f,%.3f\n", 2.5 - avg*3.3, read, chassis_power);
                    // printff("%.5f,%d,%.5f\n", 2.5 - (total * 3.3), count, chassis_power);
                // }
            }
            ...
            // printfESP("%.3f,%.3f,%.3f,%d,%d,%d,%d,%d,%d,%d,%d,%d\n", 2.5 - avg*3.3, read, chassis_power, LFpower, LBpower, RFpower, RBpower, LFrpm, LBrpm, RFrpm, RBrpm);
            // printff("%.5f,%.3f,%.3f\n",  2.5 - avg*3.3, read, chassis_power);
            ...
            double jx = remote.leftX() / 660.0; //was 660 //limited to 900
            double jy = remote.leftY() / 660.0; //was 600  // limited to 900
            double jr = remote.rightX() / 5000.0; //was 600 //limited to 5000
            ...
            // printfESP("%.5f,%d,%d,%d,%d,%d,%d,%d,%d,%d\n", 2.5 - total*3.3, LFpower, LBpower, RFpower, RBpower, LFrpm, LBrpm, RFrpm, RBrpm);
        ...
        unsigned long timeAtTheEnd = us_ticker_read();
        total = 0;
        count = 0;
        while(us_ticker_read() - timeAtTheEnd < 1000){
            total += curr.read();
            count ++;
        }
        total /= count;
        ...
        // int LFpower = abs(Chassis.getMotor(ChassisSubsystem::LEFT_FRONT).getData(POWEROUT));
        // int LBpower = abs(Chassis.getMotor(ChassisSubsystem::LEFT_BACK).getData(POWEROUT));
        // int RFpower = abs(Chassis.getMotor(ChassisSubsystem::RIGHT_FRONT).getData(POWEROUT));
        // int RBpower = abs(Chassis.getMotor(ChassisSubsystem::RIGHT_BACK).getData(POWEROUT));

        // int LFrpm = abs(Chassis.getMotor(ChassisSubsystem::LEFT_FRONT).getData(VELOCITY));
        // int LBrpm = abs(Chassis.getMotor(ChassisSubsystem::LEFT_BACK).getData(VELOCITY));
        // int RFrpm = abs(Chassis.getMotor(ChassisSubsystem::RIGHT_FRONT).getData(VELOCITY));
        // int RBrpm = abs(Chassis.getMotor(ChassisSubsystem::RIGHT_BACK).getData(VELOCITY));
        userButton = button;
    }
}
...
// DJIMotor testMot(1, CANHandler::CANBUS_1, M3508, "testbench_motor");
...
//             motorSpeed = remote.leftX() *10;

//             testMot.setPower(motorSpeed);
