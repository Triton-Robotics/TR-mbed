...
MecanumChassisSubsystem Chassis(1, 2, 3, 4, imu, 0.22617); // radius is 9 in
        ...
        if ((timeStart - loopTimer) / 1000 > OUTER_LOOP_DT_MS){
            ...
            if (printLoop >= PRINT_FREQUENCY){
                ...
                printff("Chassis: LF:%c RF:%c LB:%c RB:%c Yaw:%c Pitch:%c Flywheel_L:%c Flywheel_R:%c Indexer:%c\n", 
                    Chassis.getMotor(MecanumChassisSubsystem::LEFT_FRONT).isConnected() ? 'y' : 'n', 
                    Chassis.getMotor(MecanumChassisSubsystem::RIGHT_FRONT).isConnected() ? 'y' : 'n', 
                    Chassis.getMotor(MecanumChassisSubsystem::LEFT_BACK).isConnected() ? 'y' : 'n', 
                    Chassis.getMotor(MecanumChassisSubsystem::RIGHT_BACK).isConnected() ? 'y' : 'n',
                    yaw.isConnected() ? 'y' : 'n', 
