    ...
    while(true){
        ...
        if ((timeStart - loopTimer) / 1000 > 15){
            ...
            for (uint8_t i = 0; i < 100 ; i++) {
                data[i] = i;
                fillBufferDebug = bcJetson.write(&data[i], 1);  //result
                printf("num: %d indicate: %d \n", data[i], fillBufferDebug);
            }
            bcJetson.sync();

            // //read 9 bytes of buffer
            // readBufferDebug = bcJetson.read(&jetson_value, 9);
            // for (int i = 0 ; i < 9 ; ++i) {
            //     printf("%d ", jetson_value[i]);
            //     numBytes += 9;//++numBytes;
            //     totalBytes += 9;//++totalBytes;
            // 
            // printf("\n num: %d tot: %d indicate: %d\n\n", numBytes, totalBytes, readBufferDebug);
            // numBytes = 0;
            ...
            if (printLoop == 10){
                printf("\n----------CLEAR----------\n\n");
                while (bcJetson.readable()) {
                    buffClearDebug = bcJetson.read(&bufferClear, 1);
                    printf("%d ", bufferClear);
                    ++totalBytes;
                }
                
                //successful reading/num bytes sent
                printf("indicator: %d\n", buffClearDebug);
            ...
            printLoop ++;
            if(printLoop > 5){
                //printLoop = 0; //changed for buffer testing
