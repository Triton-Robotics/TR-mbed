...
int main(){
    ...
    */
...
#else
    float yaw_desired_angle = yaw_percent * 360.0f;
#endif
    ...
    while(true){
        ...
        float yaw_percent = encoder.read();
        float yaw_current_angle = yaw_percent * 360.0f;
        ...
        if ((timeStart - loopTimer) / 1000 > OUTER_LOOP_DT_MS){
            ...
            //YAW CODE
            if (drive == 'u' || drive == 'd' ||
                (drive =='o' && (remote.rightSwitch() == Remote::SwitchState::UP ||
                                remote.rightSwitch() == Remote::SwitchState::DOWN))) {

                chassis_rotation_radps = cs.vOmega;
                chassis_rotation_rpm = chassis_rotation_radps * 60 / (2*PI) * 0.85f; // scale factor 0.85

                yaw_desired_angle -= myaw * MOUSE_SENSITIVITY_YAW_DPS * elapsedms / 1000.0f;
                yaw_desired_angle -= jyaw * JOYSTICK_SENSITIVITY_YAW_DPS * elapsedms / 1000.0f;

                yaw_desired_angle = fmodf(fmodf(yaw_desired_angle, 360.0f) + 360.0f, 360.0f);

            #ifdef USE_IMU
                float measured_yaw = imuAngles.yaw + 180.0f;
                float error = DJIMotor::s_calculateDeltaPhaseF(
                    yaw_desired_angle, measured_yaw, 360.0f);

                yawVelo = yaw.calculatePeriodicPosition(
                    error, timeSure - prevTimeSure, chassis_rotation_rpm);
            #else
                yawVelo = -jyaw * JOYSTICK_SENSITIVITY_YAW_DPS / 360.0f * 60.0f;
                if (drive == 'd' ||
                    (drive == 'o' && remote.rightSwitch() == Remote::SwitchState::DOWN)) {
                ...
                yawVelo *= M3508_GEAR_RATIO; // rpm of post-gear shaft
                ...
                yaw.setPower(0);
            #ifdef USE_IMU
                yaw_desired_angle = imuAngles.yaw + 180.0f;
            #else
                yaw_desired_angle = yaw_current_angle;   // reset desired to current when disabled
            #endif
            }
