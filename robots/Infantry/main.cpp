...
int main(){
    ...
    */
    //YAW
    PID yawBeyblade(1, 0.00, 0.00); //yaw PID is cascading, so there are external position PIDs for yaw control
    yawBeyblade.setIntegralCap(2);
    // PID yawNonBeyblade(0.15, 0, 550);
    yawBeyblade.setOutputCap(90);
    ...
    while(true){
        ...
        if ((timeStart - loopTimer) / 1000 > OUTER_LOOP_DT_MS){
            ...
            if (drive == 'u' || (drive =='o' && remote.rightSwitch() == Remote::SwitchState::UP)){
                ...
                Chassis.setChassisSpeeds(beybladeSpeeds, 100,
                                          ChassisSubsystem::YAW_ORIENTED);
            }else{
                //OFF
                Chassis.setWheelPower({0,0,0,0});
            ...
            if (drive == 'u' || drive == 'd' || (drive =='o' && (remote.rightSwitch() == Remote::SwitchState::UP || remote.rightSwitch() == Remote::SwitchState::DOWN))){
                ...
                //Off - Explicitly stop speed control first, then set power to 0
                yaw.setSpeed(0);  // Clear any speed control first
                yaw.setPower(0);  // Then set power to 0
                #ifdef USE_IMU
                yaw_desired_angle = imuAngles.yaw + 180;
                #endif
            }
            ...
            if (drive == 'u' || drive == 'd' || (drive =='o' && (remote.rightSwitch() == Remote::SwitchState::UP || remote.rightSwitch() == Remote::SwitchState::DOWN))){
                ...
                //Off - Explicitly stop speed control first, then set power to 0
                pitch.setSpeed(0);  // Clear any speed control first  
                pitch.setPower(0);  // Then set power to 0
            }
