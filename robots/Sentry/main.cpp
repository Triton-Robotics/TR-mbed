...
bool converged, steadyState, testStart = false;
...
int main(){
    ...
    DJIMotor *testMot, *testMot2;

    yawMot1.setSpeedPID(922.9095, 0.51424, 0);
    yawMot1.setSpeedIntegralCap(5000);
...
//                    printff("POWEROUT: %d\n", powerOut);
                ...
                if (!testStart) { // initialize test vars
                ...
                if (converged && steadyState) { // set next speed once system settles
                    ...
                    testMot->pidSpeed.feedForward = (-0.431808886065578 * pow(desiredOutput, 2)) * dir
                                                    + 167.091229447692 * desiredOutput + 2643.17393278750 * dir;
                    if (velocity)
                        testMot->setSpeed(desiredOutput);
                    else if (position)
                        testMot->setPosition(desiredOutput);
                    ...
                    if (velocity) { // ss: velocity = 0
                        ...
                        if (pitch) {
                            ...
                            float limit = 0.02;
                            float curValue = (float) ((*testMot)>>ANGLE);
                            float error = (curValue - nextStartingPitch) / curValue;
                    ...
                    float limit = 0.02;
                    if (yaw && velocity) {
                        float curValue = (float) ((*testMot)>>VELOCITY);
//                        float error = (curValue - desiredOutput) / (curValue + 1e-6);
                        float error_raw = curValue - desiredOutput;
                        if (refLoop == 9)
                            printff("error: %.2f\n", error_raw);
                        if (abs(error_raw) <= 1) {
                            converged = true;
                        ...
                        float curValue = (float) ((*testMot)>>ANGLE);
                        float error = (curValue - desiredOutput) / curValue;
                        /*
...
//                                               + 167.091229447692 * desiredOutput + 1643.17393278750 * dir;
            ...
            if (refLoop >= 10) { // printing every 5 cycles
                refLoop = 0;
                ...
                if (switL && velocity)
                    printff("%d\t%d\t%d\t%d\t%d\t%d\t%d\n", speedError, desiredOutput, curSpeed, iCSpd, testMot->powerOut, testMot2->powerOut);
