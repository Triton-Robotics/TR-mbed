...
//    pitch.setPositionOutputCap(100000);
    ...
    float InitialOffset_Ticks = 6500;
    ...
    */
    ...
    while (true)
    {
        ...
        if ((timeStart - loopTimer) / 1000 > 25){
            ...
            if (shoot){
                if (indexerL>>MULTITURNANGLE <= shootTargetPosition){
                    indexerL.setSpeed(0);
                    shoot = false;
                } else {
                    timeSure = us_ticker_read();
                    indexerL.setSpeed(sure.calculate(shootTargetPosition, indexerL>>MULTITURNANGLE, timeSure - prevTimeSure)); //
                    prevTimeSure = timeSure;
                }
            } else {
                indexerL.setSpeed(0);
            ...
            if (remote.leftSwitch() == Remote::SwitchState::UP || remote.getMouseL()){
                ...
                if (shootReady){
                    shootReady = false;
                    // shoot = true;
                    shootTargetPosition = (indexerL>>MULTITURNANGLE) - 8192 * 12;
            ...
            if (remote.leftSwitch() != Remote::SwitchState::DOWN &&
                ...
                int xpeed = 7000;
                RFLYWHEEL_U.setSpeed(-xpeed);
                LFLYWHEEL_U.setSpeed(xpeed);
                RFLYWHEEL_D.setSpeed(-xpeed);
                LFLYWHEEL_D.setSpeed(xpeed);
            } else{
