...
{
public:
    AS5600(PinName p_sda, PinName p_scl, uint8_t addr, uint8_t mode);

    AS5600(I2C& p_i2c, uint8_t addr, uint8_t mode);
    ...
    float    getAngularSpeed(uint8_t mode = AS5600_MODE_DEGREES,
                            bool update = true, uint8_t dt = 15);
...
private:
    ...
    uint8_t       _mode            = AS5600_MODE_DEGREES;
