...
AS5600::AS5600(PinName p_sda, PinName p_scl, uint8_t addr, uint8_t mode):
...
AS5600::AS5600(I2C& p_i2c, uint8_t addr, uint8_t mode):
    _i2c(p_i2c)
{
    _address = addr;
    _mode = mode;
...
void AS5600::setDirection(uint8_t direction)
{
  ...
  {
    writeReg(_directionPin, _direction);
  }
}
...
uint8_t AS5600::getDirection()
{
  if (_directionPin != AS5600_SW_DIRECTION_PIN)
  {
    _direction = readReg(_directionPin);
  }
  return _direction;
}
...
void AS5600::getZPosition(int *value)
{
  *value = readReg2(AS5600_ZPOS) & 0x0FFF;
}
...
float AS5600::getAngularSpeed(uint8_t mode, bool update, uint8_t dt)
{
  if (update)
  {
...
{
    dt[0] = reg;
    _i2c.write(_address, dt, 1, true);
    _i2c.read(_address, dt, 1, false);
    return dt[0];
}
...
uint16_t AS5600::readReg2(uint8_t reg)
{
  dt[0] = reg;
  _i2c.write(_address, dt, 1, true);
  _i2c.read(_address, dt, 2, false);
  uint16_t _data = dt[1] << 8 | dt[0];
  return _data;
}
...
uint8_t AS5600::writeReg(uint8_t reg, uint8_t value)
{
    ...
    dt[0] = reg;
    dt[1] = value;
    _i2c.write(_address, dt, 2, false);
    d = dt[0];
    return d;
}
...
uint8_t AS5600::writeReg2(uint8_t reg, uint16_t value)
{
    ...
    dt[1] = value >> 8;
    dt[2] = value & 0xFF;
    _i2c.write(_address, dt, 3, false);
    d = dt[0];
    return d;
}
