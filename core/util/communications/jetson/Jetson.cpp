...
void decode_toSTM32_jetson(char *read_buf, float &received_one, float &received_two,
                           float &received_three, char &received_four, uint8_t &checksum)
{
  memcpy(&received_one, read_buf, sizeof(float));
  memcpy(&received_two, read_buf + 4, sizeof(float));
  memcpy(&received_three, read_buf + 8, sizeof(float));
  memcpy(&received_four, read_buf + 12, sizeof(char)); // calibration byte
  checksum = read_buf[13];
}
